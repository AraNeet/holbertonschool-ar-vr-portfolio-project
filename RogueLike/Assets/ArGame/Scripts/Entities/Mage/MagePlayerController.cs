...
/// <summary>
/// Main controller for the Mage player character.
...
public class MagePlayerController : MonoBehaviour
{
    ...
    public float maxHealth = 80f;           // Maximum player health (lower than Barb)
    ...
    private MageStateMachine stateMachine;      // Manages player states
    private MovementController movementController; // Handles movement
    private MageCombatController combatController; // Handles combat abilities
    ...
    void Start()
    {
        ...
        {
            stateMachine = gameObject.AddComponent<MageStateMachine>();
        }
        ...
        combatController = GetComponent<MageCombatController>();
        if (combatController == null)
        {
            combatController = gameObject.AddComponent<MageCombatController>();
        }
        ...
        if (animator != null)
        {
            ...
            Debug.Log("[MagePlayerController] Initialized attack speed multiplier");
        }
        ...
        stateMachine.Initialize(new MageIdleState(stateMachine, gameObject, movementController, animator));
    }
    ...
    void Update()
    {
        ...
        if (stateMachine.currentState is MageIdleState ||
            stateMachine.currentState is MageWalkState ||
            stateMachine.currentState is MageRunState)
        {
            // Ranged attack (left mouse button)
            if (Input.GetMouseButtonDown(0))
            {
                stateMachine.ChangeState(new MageAttackState(stateMachine, gameObject, movementController, animator, combatController));
                return;
            }
        }
    }
    ...
    private void OnPlayerDamaged()
    {
        ...
        if (!(stateMachine.currentState is MageDamagedState) &&
            !(stateMachine.currentState is MageDeathState))
        {
            stateMachine.ChangeState(new MageDamagedState(stateMachine, gameObject, movementController, animator, health));
        }
    }
    ...
    {
        stateMachine.ChangeState(new MageDeathState(stateMachine, gameObject, movementController, animator, health));
    }
    ...
    {
        Debug.Log($"[Mage] OnCollisionEnter with {collision.gameObject.name}, tag: {collision.gameObject.tag}");
    ...
    {
        Debug.Log($"[Mage] OnTriggerEnter with {other.gameObject.name}, tag: {other.gameObject.tag}");
    ...
    private void HandleEnemyCollision(GameObject collider)
    {
        ...
        {
            Debug.Log($"[Mage] Detected enemy/hitbox collision with {collider.name}");
            ...
            if (enemyAttack != null)
            {
                ...
                if (enemyAttack.CanDealDamage())
                {
                    ...
                    Debug.Log($"[Mage] Taking {damageAmount} damage from {collider.name}");
                ...
                {
                    Debug.Log($"[Mage] Enemy {collider.name} hit ignored - not in damage dealing phase");
                }
            }
            else
            {
                // If no EnemyAttack component, apply default damage
                Debug.Log($"[Mage] Taking default {damageAmount} damage from {collider.name} (no EnemyAttack component)");
                health.TakeDamage(damageAmount);
            }
        }
    }
